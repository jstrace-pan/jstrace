...
bool ScriptLoader::executeScript(const ScriptSourceCode& sourceCode)
{
    ...
    // std::cout << "[jstrace @ src/third_party/WebKit/Source/core/dom/ScriptLoader.cpp] JS URL: " <<
    // 	this->client()->sourceAttributeValue() << std::endl;
    frame->script().executeScriptInMainWorld(sourceCode, accessControlStatus, this->client()->sourceAttributeValue());
