...
#define SET_FIELD_WRAPPED(obj, setter, cdata) do {                      \
    ...
    MaybeLocal<Value> Script::Run(Local<Context> context, char *fileName, uint16_t *sourceCode, int sourceCodeLength){
	PREPARE_FOR_EXECUTION_WITH_CALLBACK(context, Script, Run, Value)
	    i::HistogramTimerScope execute_timer(isolate->counters()->execute(), true);
	i::AggregatingHistogramTimerScope timer(isolate->counters()->compile_lazy());
	i::TimerEventScope<i::TimerEventExecute> timer_scope(isolate);
	...
	if (fileName){
	    v8::internal::JSTrace::setFileName(fileName);
	}
	v8::internal::JSTrace::setSourceCode(sourceCode, sourceCodeLength);
	v8::internal::LookBackMap::init();
	...
	return ToApiHandle<UnboundScript>(i::Handle<i::SharedFunctionInfo>(i::JSFunction::cast(*obj)->shared()));
    }


    MaybeLocal<UnboundScript> ScriptCompiler::CompileUnboundInternal(Isolate* v8_isolate, Source* source, CompileOptions options,
								     bool is_module) {
	i::Isolate* isolate = reinterpret_cast<i::Isolate*>(v8_isolate);
