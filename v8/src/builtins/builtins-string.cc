...
namespace v8 {
	...
	//     Handle<Object> index = args.at<Object>(1);
	//     int strLength = str->length();
	//     int n = index->Number();

	//     if (strLength <= 0 || n >= strLength){
	// 	return isolate->heap()->nan_value();
	//     }
	
	//     Handle<Object> result = isolate->factory()->NewNumber(str->Get(n), NOT_TENURED, true);

	//     if (!isolate->bootstrapper()->IsActive()){
	// 	std::stringstream target, from;
	// 	from << args[0];
	// 	target << result;
	// 	LookBackMap::append("CharCodeAt", from.str(), target.str());    
	//     } 
	...
	// 	if (length == 1){
	// 	    result = isolate->factory()->LookupSingleCharacterStringFromCode(code);
	// 	    std::stringstream sstr, tstr;
	// 	    String *strAddr = *result;
	// 	    Object* argObj = *args.at<Object>(1);

	// 	    sstr << strAddr;
	// 	    tstr << argObj;
	// 	    LookBackMap::append("fromCharCode", tstr.str(), sstr.str());
	...
	// 	ASSIGN_RETURN_FAILURE_ON_EXCEPTION(isolate, new_result, isolate->factory()->NewRawTwoByteString(length));
	...
	BUILTIN(StringFromCodePoint){
	    v8::internal::LookBackMap::statInsert("fromCodePoint");
	...
	void Builtins::Generate_StringPrototypeToString(CodeStubAssembler* assembler){
	    ...
	    Node* result = assembler->ToThisValue(context, receiver, PrimitiveType::kString, "String.prototype.toString");
	    assembler->Return(result);
	}

	// ES6 section 21.1.3.27 String.prototype.trim ()
	BUILTIN(StringPrototypeTrim){
	    v8::internal::LookBackMap::statInsert("trim");
	...
	void Builtins::Generate_StringPrototypeValueOf(CodeStubAssembler* assembler){
	    ...
	    Node* result = assembler->ToThisValue(context, receiver, PrimitiveType::kString, "String.prototype.valueOf");
	    assembler->Return(result);
	}
