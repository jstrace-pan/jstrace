...
namespace v8 {
	...
	Node* CodeStubAssembler::StringCharCodeAt(Node* string, Node* index) {
		...
		Bind(&if_stringissequential);
		{
		    ...
		    {
			var_result.Bind(Load(MachineType::Uint8(), string,
					     IntPtrAdd(index, IntPtrConstant(SeqOneByteString::kHeaderSize -
									     kHeapObjectTag))));
			// var_result.Bind(
			// 	CallRuntime(Runtime::kStringCharCodeAt,
		...
		Bind(&if_stringisnotsequential);
		{
			...
			Bind(&if_stringisexternal);
			{
				...
				Bind(&if_stringisonebyte);
				{
				    var_result.Bind(Load(MachineType::Uint8(), string_resource_data, index));
				    // var_result.Bind(SmiFromWord32(
				...
				Bind(&if_stringistwobyte);
				{
				    ...
				    // var_result.Bind(SmiFromWord32(Load(MachineType::Uint16(), string_resource_data,
				    //                    WordShl(index, IntPtrConstant(1)))));
				    Goto(&done_loop);
				}
			    }
	...
	Node* CodeStubAssembler::StringFromCharCode(Node* code, bool forceRuntime) {

	    Variable var_result(this, MachineRepresentation::kTagged);
	    // if (forceRuntime) {
	    //   var_result.Bind(
	    //     CallRuntime(Runtime::kStringCharFromCode,
	    //       NoContextConstant(), code));
	    //   return var_result.value();
	    // }
	...
	void CodeStubAssembler::HandleLoadICHandlerCase(const LoadICParameters* p, Node* handler, Label* miss,
