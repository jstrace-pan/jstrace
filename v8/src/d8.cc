...
namespace v8 {
    ...
    ScriptCompiler::CachedData* CompileForCachedData(Local<String> source, Local<Value> name,
    ...
    MaybeLocal<Script> Shell::CompileString(Isolate* isolate, Local<String> source, Local<Value> name,
	...
	MaybeLocal<Value> maybe_result;
	{
	    ...
	    if (name->IsString()){
		Local<String> nameString = Local<String>::Cast(name);
		char fileName[nameString->Length() + 1];
		nameString->WriteUtf8(fileName);
		// char sourceString[source->Length() + 1];
		// source->WriteUtf8(sourceString);

		// std::cout << "[jstrace @ src/v8/src/d8.cc] filename: " << fileName << std::endl;
		// std::cout << "[jstrace @ src/v8/src/d8.cc] source: " << sourceString << std::endl;
		maybe_result = script->Run(realm, fileName);
	    }
	    else{
		// std::cout << "[jstrace @ src/v8/src/d8.cc] Not js file " << std::endl;
