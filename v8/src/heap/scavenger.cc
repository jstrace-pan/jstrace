...
namespace v8 {
	...
	private:
		...
		// try yacheng
		std::stringstream targetStr, sourceStr;
		targetStr << target;
		sourceStr << source;
		if (LookBackMap::contains(sourceStr.str()) && !LookBackMap::contains(targetStr.str())) {
		    LookBackMap::assign(targetStr.str(), sourceStr.str());
		    LookBackMap::remove(sourceStr.str());
		}
	...
	void Scavenger::SelectScavengingVisitorsTable() {
		...
		if (heap()->incremental_marking()->IsCompacting()) {
		    ...
		    scavenging_visitors_table_.Register(StaticVisitorBase::kVisitShortcutCandidate,
