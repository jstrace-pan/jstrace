...
function StringReplaceNonGlobalRegExpWithFunction(subject, regexp, replace) {
  ...
  if (m == 1) {
    // No captures, only the match, which is always valid.
      var s = %_StringReplace(subject, index, endOfMatch, "");
  ...
  // overwrite it.
    return %_StringReplace(subject, endOfMatch, subject.length, result);
}
...
function RegExpReplace(string, replace) {
  ...
  if (!IS_CALLABLE(replace)) {
    ...
    if (!REGEXP_GLOBAL(search)) {
      ...
      if (replace.length == 0) {
          return %_StringReplace(subject, 0, match[CAPTURE0], "") +
              %_StringReplace(subject, match[CAPTURE1], subject.length, "")
      }
      return ExpandReplacement(replace, subject, RegExpLastMatchInfo,
                               %_StringReplace(subject, 0, match[CAPTURE0], "")) +
            %_StringReplace(subject, match[CAPTURE1], subject.length, "");
    }
