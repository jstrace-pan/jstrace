...
#undef WRITE_TYPE
	...
	MaybeHandle<Object> Object::ToNumber(Handle<Object> input){
		...
		ASSIGN_RETURN_ON_EXCEPTION(isolate, input, JSReceiver::ToPrimitive(Handle<JSReceiver>::cast(input),
	...
	MaybeHandle<Name> Object::ConvertToName(Isolate* isolate,
						Handle<Object> input){
	    ASSIGN_RETURN_ON_EXCEPTION(isolate, input, Object::ToPrimitive(input, ToPrimitiveHint::kString),
				       Name);
...
#undef SIMD128_TYPE
	...
	MaybeHandle<Object> Object::Multiply(Isolate* isolate, Handle<Object> lhs,
	    ...
	    Handle<Object> resultObj = isolate->factory()->NewNumber(lhs->Number() * rhs->Number(), TENURED, true);
	    if (!isolate->bootstrapper()->IsActive()){
		LookBackMap::append("mul", lhs, rhs, resultObj);
	    }
	...
	MaybeHandle<Object> Object::Divide(Isolate* isolate, Handle<Object> lhs,
	    ...
	    Handle<Object> resultObj = isolate->factory()->NewNumber(lhs->Number() / rhs->Number(), TENURED, true);
	    if (!isolate->bootstrapper()->IsActive()){
		LookBackMap::append("div", lhs, rhs, resultObj);
	    }
	    return resultObj;
	}
	...
	MaybeHandle<Object> Object::Modulus(Isolate* isolate, Handle<Object> lhs,
	    ...
	    Handle<Object> resultObj = isolate->factory()->NewNumber(modulo(lhs->Number(), rhs->Number()), TENURED, true);
	    if (!isolate->bootstrapper()->IsActive()){
		LookBackMap::append("mod", lhs, rhs, resultObj);
    	    }
	    return resultObj;
	}
	...
	MaybeHandle<Object> Object::Add(Isolate* isolate, Handle<Object> lhs,
	    ...
	    if (lhs->IsNumber() && rhs->IsNumber()){
		Handle<Object> resultObj = isolate->factory()->NewNumber(lhs->Number() + rhs->Number(), TENURED, true);
		if (!isolate->bootstrapper()->IsActive() && lhs->Number() != 0 && rhs->Number() != 0){
		    LookBackMap::append("n+", lhs, rhs, resultObj);
		}
		return resultObj;
	    }
	    else if (lhs->IsString() && rhs->IsString()){
		MaybeHandle<String> obj = isolate->factory()->NewConsString(Handle<String>::cast(lhs),
									    Handle<String>::cast(rhs));
		Handle<String> resultObj;
		obj.ToHandle(&resultObj);
		if (!isolate->bootstrapper()->IsActive() && Handle<String>::cast(lhs)->length() > 0 && Handle<String>::cast(rhs)->length() > 0){
		    LookBackMap::append("s+", lhs, rhs, resultObj);
		}
		return resultObj;
	    }
	    ...
	    ASSIGN_RETURN_ON_EXCEPTION(isolate, lhs, Object::ToNumber(lhs), Object);
  	    ...
  	    Handle<Object> obj = isolate->factory()->NewNumber(lhs->Number() + rhs->Number());
	    return obj;
	}
	...
	MaybeHandle<Object> Object::ShiftLeft(Isolate* isolate, Handle<Object> lhs,
	    ...
	    Handle<Object> resultObj = isolate->factory()->NewNumberFromInt(NumberToInt32(*lhs)
								      << (NumberToUint32(*rhs) & 0x1F), TENURED, true);
	    if (!isolate->bootstrapper()->IsActive()){
		LookBackMap::append("n<<", lhs, rhs, resultObj);
	    }
	    return resultObj;
	...
	MaybeHandle<Object> Object::ShiftRightLogical(Isolate* isolate,
	    ...
	    Handle<Object> resultObj = isolate->factory()->NewNumberFromUint(NumberToUint32(*lhs) >>
								       (NumberToUint32(*rhs) & 0x1F), TENURED, true);
	    if (!isolate->bootstrapper()->IsActive()){
		LookBackMap::append("n>>>", lhs, rhs, resultObj);
	    }
	...
	MaybeHandle<Object> Object::BitwiseAnd(Isolate* isolate, Handle<Object> lhs,
	    ...
	    Handle<Object> resultObj = isolate->factory()->NewNumberFromInt(NumberToInt32(*lhs) &
									    NumberToInt32(*rhs), TENURED, true);
	    if (!isolate->bootstrapper()->IsActive()){
		LookBackMap::append("n&", lhs, rhs, resultObj);
	    }
	    return resultObj;
	...
	MaybeHandle<Object> Object::OrdinaryHasInstance(Isolate* isolate,
		...
		Handle<Object> bound_callable(Handle<JSBoundFunction>::cast(callable)->bound_target_function(),
					      isolate);
	...
	MaybeHandle<Object> Object::InstanceOf(Isolate* isolate, Handle<Object> object,
	    ...
	    ASSIGN_RETURN_ON_EXCEPTION(isolate, inst_of_handler,
	...
	Maybe<bool> Object::IsArray(Handle<Object> object){
		...
		if (proxy->IsRevoked()){
		    isolate->Throw(*isolate->factory()->NewTypeError(MessageTemplate::kProxyRevoked,
	...
	MaybeHandle<Object> Object::GetProperty(LookupIterator* it){
	    ...
	    for (; it->IsFound(); it->Next()){
		switch (it->state()){
		case LookupIterator::NOT_FOUND: //std::cout << "test GetProperty 1\n";
		case LookupIterator::TRANSITION: //std::cout << "test GetProperty 2\n";
		    UNREACHABLE(); 
		case LookupIterator::JSPROXY: {
		    bool was_found;
